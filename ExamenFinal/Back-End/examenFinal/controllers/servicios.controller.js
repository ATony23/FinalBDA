...
async function mostrarMedicos(req, res) {
...
async function agregar(req, res) {
   ...
   const { name, description, inCharge, cost, address, rate } = req.body;

   if (!name || !description || !inCharge || !cost || !address || !rate) return res.status(404).json({ "Success": false, "Message": "Missing Variables" });
